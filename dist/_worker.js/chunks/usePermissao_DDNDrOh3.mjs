...
import { s as supabase } from './systemName_EsfuoaVO.mjs';
  ...
  reactExports.useEffect(() => {
    ...
    async function carregarPermissao() {
      ...
      try {
        ...
        const { data, error } = await supabase.from("modulo_acesso").select("permissao, ativo").eq("usuario_id", user.id).ilike("modulo", moduloTrimmed);
        if (cancelled) return;
        if (error) {
